...
public class MainContext : ApplicationContext
{
    ...
    private bool _comAvailable = false; // 标记 COM 接口是否可用
...
• 多屏轮流切换：按 A→B→C→A 的顺序每间隔N秒更换一个屏幕的壁纸（需要Windows 10+支持独立壁纸）。
